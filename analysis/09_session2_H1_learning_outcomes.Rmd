...
```{r fig.width=12}
ggplot(block2_cutoff_stats, aes(y = unique_items, x = condition, group = condition)) +
...
ggsave("../output/number-of-facts-session2-cutoff-simple.pdf", device = "pdf", width = 5, height = 3)
```
...
```{r fig.width=12}
ggplot(test2_cutoff_stats, aes(y = n_correct, x = condition, group = condition)) +
  geom_jitter(width = 0.1, height = 0, alpha = 0.5, aes(colour = condition)) +
  geom_violin(fill = NA) +
  geom_boxplot(width = 0.2, outlier.shape = NA, fill = NA) +
  guides(colour = FALSE) +
...
ggsave("../output/test-score-session2-cutoff-simple.pdf", device = "pdf", width = 5, height = 3)
```
...
tikz(file = "../output/test-accuracy-session2-full-simple.tex", width = 4.75, height = 2)

test2_full_stats_tex %>%
