...
Recalculate these Bayes factors using the intercept model as reference for the table in the paper:
```{r}
bayes_factor(m1_noblock, m1_intercept)$bf # condition
bayes_factor(m1_nocond, m1_intercept)$bf # block
bayes_factor(m1, m1_intercept)$bf # condition + block
bayes_factor(m1_X, m1_intercept)$bf # condition * block
```
...
Plot the model predictions per block:
```{r}
p <- ggplot(pred_acc_test, aes(x = alpha, y = fit, colour = condition, fill = condition)) +
  ...
  scale_y_continuous(limits = c(.5, 1), labels = scales::percent_format()) +
...
pred_acc_test_simple[, stage := "Delayed recall test"]
...
test_acc_avg <- test_studied_alpha %>%
  ...
  mutate(stage = "Delayed recall test")
...
p <- ggplot(data = pred_acc_combi_simple, aes(x = alpha, y = fit, colour = condition, fill = condition)) +
  ...
  geom_point(data = acc_avg, aes(y = accuracy), alpha = .5) +
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.25, colour = NA) +
  geom_line() +
  scale_colour_manual(NULL, values = condition_colours) +
  scale_fill_manual(NULL, values = condition_colours) +
  scale_x_continuous(limits = c(min(acc_avg$alpha), max(acc_avg$alpha))) +
...
saveRDS(p, "../output/combi-accuracy-rof.rds")
saveRDS(p_legend, "../output/legend.rds")
...
facet_labels <- c("Learning session", "Delayed recall test")
names(facet_labels) <- c("Learning session", "Test")

ggplot(data = pred_acc_combi_simple, aes(x = alpha, y = fit, colour = condition, fill = condition)) +
  facet_grid(~ stage, labeller = labeller(stage = facet_labels)) +
  geom_linerange(data = acc_avg, aes(y = NULL, ymin = accuracy - accuracy_se, ymax = accuracy + accuracy_se), alpha = .15) +
  geom_point(data = acc_avg, aes(y = accuracy), size = .2, alpha = .5) +
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.25, colour = NA) +
  geom_line() +
  scale_colour_manual(NULL, values = condition_colours, labels = c("Cold start", "Warm start")) +
  scale_fill_manual(NULL, values = condition_colours, labels = c("Cold start", "Warm start")) +
  # scale_y_continuous(limits = c(.5, 1), labels = scales::percent_format(suffix = "\\%")) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  # guides(colour = guide_legend(reverse = TRUE),
  #        fill = guide_legend(reverse = TRUE)) +
...
ggsave("../output/combi-accuracy-presentation.png", device = "png", width = 9, height = 4, bg = "transparent")
...
p <- ggplot(pred_acc, aes(x = alpha, y = fit, colour = condition, fill = condition)) +
  ...
  labs(x = "Predicted fact-level rate of forgetting",  y = "Presentations") +
  guides(colour = FALSE,
         fill = FALSE) +
  theme_bw(base_size = 12) + 
  theme(axis.text = element_text(colour = "black"),
...
saveRDS(p, "../output/presentations-rof.rds")
...
Also make this plot for the presentation:
```{r}
ggplot(pred_acc, aes(x = alpha, y = fit, colour = condition, fill = condition)) +
  geom_errorbar(data = presentations_mean, aes(y = reps_mean, ymin = reps_mean - reps_se, ymax = reps_mean + reps_se), alpha = .15) +
  geom_point(data = presentations_mean, aes(y = reps_mean), alpha = .5) +
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.25, colour = NA) +
  geom_line() +
...
ggsave("../output/number-presentations-presentation.png", device = "png", width = 5.5, height = 3, bg = "transparent")
...
rl_acc <- s2_rl %>%
  ...
  summarise(p_corr = mean(correct[study == FALSE]))
```
...
saveRDS(p, "../output/learning-accuracy.rds")
...
```{r}
contrasts(s2_rl$condition) <- c(-0.5, 0.5)
contrasts(s2_rl$condition)

contrasts(s2_rl$block) <- c(-0.5, 0.5)
contrasts(s2_rl$block)
```

Set the priors for the fixed effects:
```{r}
prior_m_learn <- set_prior("cauchy(0, 1)", class = "b")
```

Remove study trials from the data:
```{r}
s2_rl_nostudy <- filter(s2_rl, study == FALSE)
```


Fit the maximal model, along with simpler models:
```{r}
m_learn_1 <- brm(
  correct ~ condition * block + (1 | subject) + (1 | fact_id),
  family = bernoulli,
  ...
  future = TRUE,
  seed = 0,
  file = "model_fits/m_learn_1"
)
```

Intercept model (used as reference):
```{r}
m_learn_5 <- brm(
  correct ~ 1 + (1 | subject) + (1 | fact_id),
  family = bernoulli,
  ...
  future = TRUE,
  seed = 0,
  file = "model_fits/m_learn_5"
)
```

Compare using bridge sampling:
```{r}
bf_1v5 <- bayes_factor(m_learn_1, m_learn_5)
bf_1v5$bf
```
...
m_learn_2 <- brm(
  ...
  future = TRUE,
  seed = 0,
  file = "model_fits/m_learn_2"
)
```

```{r}
bf_2v5 <- bayes_factor(m_learn_2, m_learn_5)
bf_2v5$bf
```
...
m_learn_3 <- brm(
  ...
  future = TRUE,
  seed = 0,
  file = "model_fits/m_learn_3"
)
```

```{r}
bf_3v5 <- bayes_factor(m_learn_3, m_learn_5)
bf_3v5$bf
```
...
m_learn_4 <- brm(
  ...
  future = TRUE,
  seed = 0,
  file = "model_fits/m_learn_4"
)
```

```{r}
bf_4v5 <- bayes_factor(m_learn_4, m_learn_5)
bf_4v5$bf
```
...
The best model among these options is the one with additive effects (but no interaction) of condition and block.
```{r}
m_learn_2 <- readRDS("model_fits/m_learn_2.rds")
```


```{r, fig.width = 12}
mcmc_plot(m_learn_2, pars = c("condition", "block"), type = "trace")

mcmc_plot(m_learn_2, pars = c("condition", "block"), type = "acf_bar")
```
...
Visualise the model's population-level estimates, along with their 95% credible intervals:
```{r}
mcmc_plot(m_learn_2, pars = c("condition", "block"), type = "areas", prob = .95)
```
...
fit_m_learn_2 <- data.table(fitted(m_learn_2,
                            newdata = new_data,
                            re_formula = NA,
                            summary = FALSE))
setnames(fit_m_learn_2, as.character(interaction(new_data$condition, new_data$block)))

fit_m_learn_2_summary <- data.table(fitted(m_learn_2,
                                    newdata = new_data,
                                    re_formula = NA))
...
```{r}
quantile((fit_m_learn_2$Default.1 + fit_m_learn_2$Default.2 + fit_m_learn_2$Fact.1 + fit_m_learn_2$Fact.2)/4,
         probs = c(.5, .025, .975))
```

Difference in response accuracy between Fact condition and Default condition (across blocks): 
```{r}
fact_vs_default <- (fit_m_learn_2$Fact.1 + fit_m_learn_2$Fact.2)/2 - (fit_m_learn_2$Default.1 + fit_m_learn_2$Default.2)/2
quantile(fact_vs_default, probs = c(.5, .025, .975))
```

Difference in response accuracy between block 2 and block 1 (across conditions):
```{r}
block2_vs_block1 <- (fit_m_learn_2$Default.2 + fit_m_learn_2$Fact.2)/2 - (fit_m_learn_2$Default.1 + fit_m_learn_2$Fact.1)/2
...
saveRDS(p, "../output/test-score.rds")
...
```{r}
contrasts(test_scores$condition) <- c(-0.5, 0.5)
contrasts(test_scores$block) <- c(-0.5, 0.5)
```

Fit the models:
```{r}
m_test_scores_1 <- brm(score ~ block * condition + (1 | subject),
                       family = poisson(),
                       data = test_scores,
                       ...
                       future = TRUE,
                       seed = 0,
                       file = "model_fits/m_test_scores_1"
)
```

```{r}
m_test_scores_2 <- brm(score ~ block + condition + (1 | subject),
                       family = poisson(),
                       data = test_scores,
                       prior = set_prior("cauchy(0, 1)", class = "b"),
                       chains = 4,
                       ...
                       future = TRUE,
                       seed = 0,
                       file = "model_fits/m_test_scores_2"
)
```

```{r}
m_test_scores_3 <- brm(score ~ block + (1 | subject),
                       family = poisson(),
                       data = test_scores,
                       prior = set_prior("cauchy(0, 1)", class = "b"),
                       chains = 4,
                       ...
                       future = TRUE,
                       seed = 0,
                       file = "model_fits/m_test_scores_3"
)
```

```{r}
m_test_scores_4 <- brm(score ~ condition + (1 | subject),
                       family = poisson(),
                       data = test_scores,
                       prior = set_prior("cauchy(0, 1)", class = "b"),
                       chains = 4,
                       ...
                       future = TRUE,
                       seed = 0,
                       file = "model_fits/m_test_scores_4"
)
```

```{r}
m_test_scores_5 <- brm(score ~ 1 + (1 | subject),
                       family = poisson(),
                       data = test_scores,
                       chains = 4,
                       iter = 10000,
                       ...
                       future = TRUE,
                       seed = 0,
                       file = "model_fits/m_test_scores_5"
)
```
...
Compare using bridge sampling:
```{r}
bayes_factor(m_test_scores_1, m_test_scores_5)
bayes_factor(m_test_scores_2, m_test_scores_5)
bayes_factor(m_test_scores_3, m_test_scores_5)
bayes_factor(m_test_scores_4, m_test_scores_5)
```

The best model:
```{r}
summary(m_test_scores_3)
```

Visualise the model's population-level estimates, along with their 95% credible intervals:
```{r}
mcmc_plot(m_test_scores_3, pars = c("b_Intercept", "block"), type = "areas", prob = .95)
```
...
```{r}
# Get fitted values
new_data <- crossing(block = levels(test_scores$block))
fit_score <- data.table(fitted(m_test_scores_3,
                            newdata = new_data,
                            re_formula = NA,
                            summary = FALSE))
setnames(fit_score, paste0("block", new_data$block))
```


Overall test score (across blocks and conditions):
```{r}
quantile((fit_score$block1 + fit_score$block2)/2,
         probs = c(.5, .025, .975))
```


Difference in score between block 2 and block 1 (across conditions):
```{r}
block2_vs_block1 <- fit_score$block2  - fit_score$block1
quantile(block2_vs_block1, probs = c(.5, .025, .975))
```
...
Experiment 1 showed that the different prediction methods all outperformed the default prediction in terms of accuracy (RMSE between predicted and observed rate of forgetting), but that more fine-grained methods did equally well as the domain-level prediction.
...
pred_vs_obs <- test_studied_alpha_obs %>%
  mutate(predicted = ifelse(condition == "fact", alpha, 0.3)) %>%
  mutate(condition = stringr::str_to_title(condition))

pred_acc <- pred_vs_obs %>%
  group_by(condition, block, subject) %>%
  summarise(rmse = rmse(predicted, final_alpha)) %>%
  ungroup() %>%
  mutate(condition = as.factor(condition))

pred_acc_by_cond <- pred_acc %>%
  group_by(condition) %>%
...
Make the plot shown in the paper:
```{r}
p <- ggplot(pred_vs_obs, aes(x = predicted, y = final_alpha, colour = condition)) +
  ...
  geom_text(aes(label = paste("RMSE =", formatC(rmse_mean, digits = 3, flag = "#"))), x = max(test_studied_alpha_obs$final_alpha), y = min(test_studied_alpha_obs$final_alpha), hjust = 1, colour = "black", size = rel(2), data = pred_acc_by_cond) +
  coord_fixed(ratio = 1) +
  scale_x_continuous(limits = range(test_studied_alpha_obs$final_alpha)) +
  scale_y_continuous(limits = range(test_studied_alpha_obs$final_alpha)) +
  scale_colour_manual(values = condition_colours) +
  guides(colour = FALSE) +
...
saveRDS(p, "../output/predictions_rmse.rds")

p
```

It looks like RMSE was once again lower in the Fact condition, particularly in the first block.
```{r}
ggplot(pred_acc, aes(x = interaction(condition, block), y = rmse)) + 
  geom_boxplot() + 
  theme_poster
```

And an exploratory Bayesian t-test agrees:
```{r}
ttestBF(filter(pred_acc, condition == "Fact")$rmse, 
        filter(pred_acc, condition == "Default")$rmse,
...
```{r}
bf_pred_acc <- anovaBF(
  rmse ~ condition * block,
  data = pred_acc,
  progress = FALSE
)
```

Evidence for this model over a null model (no effect of condition or block):
```{r}
bf_pred_acc
```

Evidence for a null model (no effect of condition or block):
```{r}
1/bf_pred_acc
```
...
Mean prediction accuracy per condition:
```{r}
pred_acc %>%
...
Is the number of distinct facts that participants encountered during the learning session different?

```{r}
distinct_facts <- s2_rl %>%
  group_by(subject, condition, block) %>%
  summarise(n_distinct = length(unique(fact_id)))
```

```{r}
bf_distinct <- anovaBF(
  n_distinct ~ condition * block,
  data = distinct_facts,
  progress = FALSE
)
```
...
Use sum-to-zero contrasts:
```{r}
contrasts(distinct_facts$condition) <- c(-0.5, 0.5)
contrasts(distinct_facts$block) <- c(-0.5, 0.5)
```

Fit the models:
```{r}
m_facts_1 <- brm(n_distinct ~ block * condition + (1 | subject),
                 family = poisson(),
                 data = distinct_facts,
                 prior = set_prior("cauchy(0, 1)", class = "b"),
                 chains = 4,
...
```{r}
m_facts_2 <- brm(n_distinct ~ block + condition + (1 | subject),
                 family = poisson(),
                 data = distinct_facts,
                 prior = set_prior("cauchy(0, 1)", class = "b"),
                 chains = 4,
...
```{r}
m_facts_3 <- brm(n_distinct ~ block + (1 | subject),
                 family = poisson(),
                 data = distinct_facts,
                 prior = set_prior("cauchy(0, 1)", class = "b"),
                 chains = 4,
...
```{r}
m_facts_4 <- brm(n_distinct ~ condition + (1 | subject),
                 family = poisson(),
                 data = distinct_facts,
                 prior = set_prior("cauchy(0, 1)", class = "b"),
                 chains = 4,
...
```{r}
m_facts_5 <- brm(n_distinct ~ 1 + (1 | subject),
                 family = poisson(),
                 data = distinct_facts,
                 chains = 4,
                 iter = 10000,
...
Visualise the model's population-level estimates, along with their 95% credible intervals:
```{r}
mcmc_plot(m_facts_3, pars = c("b_Intercept", "block"), type = "areas", prob = .95)
```
...
```{r}
# Get fitted values
new_data <- crossing(block = levels(distinct_facts$block))
fit_facts <- data.table(fitted(m_facts_3,
                            newdata = new_data,
                            re_formula = NA,
                            summary = FALSE))
setnames(fit_facts, paste0("block", new_data$block))
```


Overall test score (across blocks and conditions):
```{r}
quantile((fit_facts$block1 + fit_facts$block2)/2,
         probs = c(.5, .025, .975))
```
...
Difference in score between block 2 and block 1 (across conditions):
```{r}
block2_vs_block1 <- fit_facts$block2  - fit_facts$block1
quantile(block2_vs_block1, probs = c(.5, .025, .975))
```
