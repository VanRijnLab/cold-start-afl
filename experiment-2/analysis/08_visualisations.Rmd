...
```{r}
p <- pred_map %>%
  mutate(category = as.factor(ifelse(category == "big", "Big cities", "Small towns"))) %>%
  ggplot(aes(x = category, y = predicted)) +
  geom_hline(yintercept = 0.3, lty = 2) +
  geom_jitter(width = 0.1, height = 0, alpha = 0.3, aes(colour = category), colour = condition_colours[2]) +
...
tikz(file = "../output/predicted-rof-exp2.tex", width = 4.75/2, height = 2)
p
dev.off()
